...
function getBuildSizes(): void {
  ...
  const moduleFiles = files.filter((f) => !f.startsWith('react-understate.') && f.endsWith('.js'));
  ...
  if (bundleFiles.length > 0) {
    ...
    bundleFiles.forEach((file: string) => {
      ...
      console.log(`  ${file.padEnd(35)} ${formatBytes(size).padStart(12)} (gzip: ${formatBytes(gzipSize).padStart(8)}, brotli: ${formatBytes(brotliSize).padStart(8)})`);
    });
    console.log('');
  }
  ...
  const sourceMapFiles = fs.readdirSync(distPath).filter((f: string) => f.endsWith('.map'));
  if (sourceMapFiles.length > 0) {
    ...
    console.log(`  Total Source Maps:${' '.repeat(20)} ${formatBytes(sourceMapSize).padStart(12)}`);
    console.log('');
  }

  // Check for TypeScript declarations
  const declarationFiles = fs.readdirSync(distPath).filter((f: string) => f.endsWith('.d.ts'));
  if (declarationFiles.length > 0) {
    ...
    console.log(`  Total Declarations:${' '.repeat(20)} ${formatBytes(declarationSize).padStart(12)}`);
    console.log('');
  }

  console.log('─'.repeat(80));
  console.log(`  Bundle Total:${' '.repeat(22)} ${formatBytes(bundleSize).padStart(12)} (gzip: ${formatBytes(estimateGzipSize(bundleSize)).padStart(8)}, brotli: ${formatBytes(estimateBrotliSize(bundleSize)).padStart(8)})`);
  console.log(`  All Files Total:${' '.repeat(19)} ${formatBytes(totalSize).padStart(12)} (gzip: ${formatBytes(totalGzipSize).padStart(8)}, brotli: ${formatBytes(totalBrotliSize).padStart(8)})`);
  ...
  } else if (bundleSize > 50 * 1024) { // > 50KB
