...
module.exports = {
  meta: {
    ...
    docs: {
      description:
        'Prefer immutable updates for object states using object spread',
    ...
    schema: [],
    messages: {
      preferObjectSpreadForUpdates:
        'Prefer immutable object updates using spread operator instead of direct property assignment.',
    },
  },
  ...
  create(context) {
    ...
    return {
      AssignmentExpression(node) {
        if (
          isStatePropertyAssignment(node) ||
          isNestedStatePropertyAssignment(node)
        ) {
