...
export const advancedFilteredTodos = derived(
  () => {
    ...
    return allTodos.filter(todo => {
      ...
      // Date range filter
      if (dateRangeValue.start && todo.createdAt < dateRangeValue.start)
        return false;
      if (dateRangeValue.end && todo.createdAt > dateRangeValue.end)
        return false;
      ...
      if (
        todoPriority < priorityRangeValue.min ||
        todoPriority > priorityRangeValue.max
      ) {
        return false;
      }
...
// Filter actions
export const setDateRange = action(
...
export const setPriorityRange = action(
