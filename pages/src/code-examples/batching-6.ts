...
export const createBatchingDebugger = () => {
  ...
  const debugBatch = (fn: () => void) => {
    ...
    console.log(
      `Batch completed: ${updateCount} updates in ${duration.toFixed(2)}ms`,
    );
    console.groupEnd();
  ...
  const createDebugState = <T>(initialValue: T, name: string) => {
    ...
    return (value: T | ((prev: T) => T)) => {
      updateCount++;
      console.log(
        `Update #${updateCount} in ${isInBatch ? 'batch' : 'individual'}: ${name}`,
      );
  ...
  return {
    debugBatch,
    createDebugState,
    getStats: () => ({ batchCount, updateCount }),
  };
};
...
export const createRealtimeMonitor = () => {
  ...
  const stopMonitoring = () => {
    ...
    console.log(
      `Batching Efficiency: ${((batchedUpdates / monitor.updates.length) * 100).toFixed(1)}%`,
    );
  };
