...
export const updateFilterAndSort = action(
  (newFilter: string, newSortBy: string) => {
...
export const addItemAndUpdateStats = action(
  (item: Item) => {
...
const createMonitoredDerived = <T>(computation: () => T, name: string) => {
  ...
  return derived(
    () => {
      ...
      if (duration > 5) {
        ...
        console.warn(
          `Derived "${name}" calculation #${calculationCount} took ${duration.toFixed(2)}ms`,
        );
      }
