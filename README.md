...
Ensures you call `useSubscribe()` for any state used in React components.

```tsx
// ❌ Incorrect
function Counter() {
  return <div>Count: {count.value}</div>; // Missing useSubscribe
}
...
Suggests using `batch()` for multiple state updates.
...
Prevents creating states inside React components.
...
Suggests using `derived()` for computed values.
...
| ----------------------------------------- | --------------------------------------------------------------- | ------- |
| `require-use-subscribe`                   | Ensure `useSubscribe()` is called for states used in components | `error` |
| `prefer-batch-for-multiple-updates`       | Suggest `batch()` for multiple state updates                    | `warn`  |
| `no-direct-state-assignment`              | Prevent assigning states to variables                           | `error` |
| `no-state-creation-in-components`         | Prevent creating states inside components                       | `error` |
| `no-nested-effects`                       | Prevent nested `effect()` calls                                 | `error` |
| `no-nested-derived`                       | Prevent nested `derived()` calls                                | `error` |
| `prefer-derived-for-computed`             | Suggest `derived()` for computed values                         | `warn`  |
| `prefer-effect-for-side-effects`          | Suggest `effect()` for side effects                             | `warn`  |
| `no-unused-states`                        | Detect unused states                                            | `warn`  |
